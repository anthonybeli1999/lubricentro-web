...
import { agregarNuevoVehiculo } from '@/app/services/vehiculosService'
...
export default function AgregarVehiculo() {
  ...
  const agregarCliente = async () => {
    ...
    const respuesta = await agregarNuevoVehiculo(body);
    if (respuesta) {
  ...
  const validarCampos = () => {
    return !(!campos.placa || !campos.tipo || !campos.marca
      || !campos.modelo || !campos.idcliente
    )
  }
  ...
  return (
    <div className="w-full max-w-screen-xl mx-auto grid grid-cols-1 gap-2 px-4">
      <div className="flex justify-between items-center pt-4 pb-2">
        <h1 className="text-lg lg:text-xl font-semibold">Agregar vehiculo</h1>
      </div>
      ...
      <div className="bg-white rounded-lg p-6" onClick={() => !mostrarAlerta ? '' : verAlerta()}>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          <div className='text-center md:pl-2 md:pr-4 mt-4 md:mt-2'>
            ...
            <InputNumber
              value={campos.anio} placeholder={'Año'}
