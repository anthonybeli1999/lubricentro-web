...
import { modificarServicio } from '@/app/services/serviciosService';
import CheckIcon from '@heroicons/react/24/outline/CheckIcon';
import XMarkIcon from '@heroicons/react/24/outline/XMarkIcon';
import { useRouter } from 'next/navigation';
import React, { useEffect, useState } from 'react'

export default function ModificarServicio() {
  ...
  const modificarServicioSeleccionado= async () => {
    if (!validarCampos()) {
      verAlerta();
      return;
    }
    ...
    const respuesta = await modificarServicio(body);
    if (respuesta) {
      alert('La información fue guardada correctamente');
      router.push('/admin/Servicios');
      localStorage.removeItem('servicio');
    } else {
      alert('Algo salió mal al modificar el servicio');
    }
  }

  const validarCampos = () => {
    return !(!campos.id || !campos.tipo || !campos.servicio || !campos.detalle
  ...
  useEffect(() => {
    const servicioString = localStorage.getItem('servicio');
    if (servicioString) {
      const servicioObj = JSON.parse(servicioString);
      setCampos({
        ...campos,
        id: servicioObj.id,
        tipo: servicioObj.Tipo,
        servicio: servicioObj.Servicio,
        detalle: servicioObj.Detalle,
        kma: servicioObj.KmA,
        kmpc: servicioObj.KmPC,
        fecha: servicioObj.FechaIngreso,
        monto: servicioObj.Monto,
        estadopago: servicioObj.EstadoPago,
        montopago: servicioObj.MontoPago,
        personal: servicioObj.Personal,
        idvehiculo: servicioObj.vehiculoId
      });
    }
  }, []);
  ...
  return (
    ...
    <div className="flex justify-between items-center pt-4 pb-2">
        <h1 className="text-lg lg:text-xl font-semibold">Modificar servicio</h1>
    </div>
    <div className="bg-white rounded-lg p-6" onClick={() => !mostrarAlerta ? '' : verAlerta()}>
        ...
        <div className='grid md:flex mt-8 mb-4 justify-center'>
            <ButtonIcon texto='Guardar' icono={<CheckIcon className="w-5" />} onClick={() => modificarServicioSeleccionado()} />
